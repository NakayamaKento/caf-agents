...
## クラウド導入戦略フェーズへの運用チームの貢献

運用チームが持つ「現行環境の実績データ」は Strategy フェーズのビジネスケース作成において最も価値が高い入力となります。

### Strategy フェーズ: 現行環境の実績データ提供

| 提供するデータ | 活用先 | 効果 |
|---|---|---|
| **現行 SLA 実績レポート** | TCO 計算の「リスク回避コスト」に反映 | ビジネスケースの信頼性向上 |
| **インシデント頻度・原因分類** | 移行後の改善効果の定量化根拠 | ROI 試算の精度向上 |
| **運用コスト明細**（人件費・ツールライセンス・オンコール） | 5 年 TCO 比較に反映 | 隠れたコストの可視化 |
| **オンプレ環境の可観測性ギャップ** | クラウド移行後の監視改善価値の定量化 | ビジネス成果の具体化 |

> @cloud-strategy との連携: Strategy フェーズのキックオフ時に現行環境の運用実績レポートを共有し、TCO 分析の入力データとして活用してもらいます。

### Plan フェーズ: 移行計画への運用観点の追加

| 貢献内容 | 連携先 | 成果物 |
|---|---|---|
| **ワークロード別運用複雑性スコアリング** | @cloud-strategy（5R 分類への追加観点） | 運用観点を加えた優先順位マトリックス |
| **Day-2 運用コスト見積もり** | @cloud-strategy（ロードマップのコスト） | 各フェーズの運用コスト計画 |
| **SLO/SLA の初期目標設定** | @cloud-strategy（ビジネス成果 KPI） | ビジネス目標から逆算した初期 SLO 案 |

### Ready フェーズ: 監視基盤の先行構築

Landing Zone 展開と並行して監視基盤を先行構築し、移行効果を測定可能にします。

- **Log Analytics Workspace のテンプレート化**: Landing Zone 展開時に自動で監視基盤を構築
- **ベースライン計測の開始**: ワークロード移行前から計測し、移行効果の比較基準を確立
- **アラートルールテンプレートの整備**: 移行後の即時監視開始を実現
