...
ガバナンスチームは CAF Plan フェーズの各成果物に以下の形で具体的に貢献します。

### Strategy 成果物 → Plan 成果物への接続

| Strategy 成果物 | ガバナンスへの接続 | Plan 成果物 |
|---|---|---|
| **リスク許容度の定義** | リスクカテゴリ別の許容しきい値 | ポリシー強制レベル選定計画（Audit/Deny ロードマップ） |
| **コンプライアンス要件の優先順位** | 規制別ガードレール要件の整理 | Policy as Code 実装ロードマップ（フェーズ別） |
| **ビジネスケースの優先ワークロード** | 高優先ワークロードのガバナンス要件定義 | ワークロード別ガバナンスチェックリスト |

### Plan フェーズ主要成果物への貢献

#### 1. クラウド導入計画（Cloud Adoption Plan）

- @cloud-strategy が定義した動機（マイグレーション/イノベーション）に応じたガバナンス適用スコープの確定
- Audit → Deny 移行の段階的タイムラインの策定（ウェーブ移行と連動）
- 各移行ウェーブのゲーティング基準（ガバナンス準拠確認チェックリスト）の提供

#### 2. スキル準備計画（Skills Readiness Plan）

- Azure Policy / RBAC / Cost Management / FinOps に関する社内スキルギャップ分析
- Policy as Code（Bicep / Terraform）習熟ロードマップの策定
- ガバナンスレビュアー育成計画と認定基準の設定

#### 3. デジタルエステート計画（Digital Estate Plan）

- 既存リソースのコンプライアンス状態評価（Resource Graph ベースラインスキャン）
- 5R 分類へのガバナンス複雑性スコア追加（例外申請多発リソースは Refactor 推奨）
- 移行後に適用するガバナンスポリシーの事前マッピング

#### 4. 組織アライメント（Organizational Alignment）

- RACI マトリックスへのガバナンスチームの役割の明記
- ポリシー例外申請・承認フローの設計と関係者合意の取得
- ガバナンスレビューサイクル（月次コンプライアンスレポート・四半期成熟度評価）のカレンダー設定

### 戦略→計画の連携フロー

```
@cloud-strategy（Strategy フェーズ）
    ├── 動機・優先順位 ──────────→ ガバナンス適用スコープの確定
    ├── リスク許容度 ────────────→ Audit/Deny ポリシー選定ロードマップ
    └── コンプライアンス要件 ───→ Policy as Code 実装計画
            │
            ▼
@cloud-governance（Plan フェーズ）
    ├── ポリシーイニシアティブ定義 → @cloud-platform（IaC 実装）
    ├── 例外申請フロー定義 ────→ @ccoe（セルフサービスカタログ）
    └── 成熟度目標設定 ─────→ @ccoe（進捗モニタリング）
```

> @cloud-strategy との連携: Strategy フェーズのキックオフ時に「リスク許容度定義書」と「コンプライアンス要件優先順位表」を受領し、それを基にガバナンスポリシーの Plan フェーズ計画を策定します。
