...
### Microsoft Defender for Cloud プランのコスト対効果

セキュリティ強化は費用対効果を考慮した段階的導入を推奨します。

| Defender プラン | 月額コスト（概算） | 保護対象 | ROI 判断基準 |
|---|---|---|---|
| **Defender for Servers P2** | $15/サーバー/月 | VM | EDR + 脆弱性評価が必要な本番環境では必須 |
| **Defender for Storage** | $0.02/10万トランザクション | ストレージ | 機密データを含むストレージには必須 |
| **Defender for SQL** | $15/サーバー/月 | Azure SQL | 個人情報・財務データを扱うDBに必須 |
| **Defender CSPM（有償）** | $0.007/リソース/月 | 全リソース | セキュアスコア 70% 未満の環境では ROI 高 |
| **Defender for Containers** | $7/vCore/月 | AKS | 本番 Kubernetes 環境では強く推奨 |

> 💡 **価格は参考値です（2024 年時点の概算）**: 上記コストはリージョン・構成・割引契約により変動します。価格は頻繁に改定されるため、実際の計画策定時は必ず [Azure 公式価格ページ](https://azure.microsoft.com/ja-jp/pricing/details/defender-for-cloud/) で最新情報を確認してください。
...
| セキュリティ施策 | セキュリティ効果 | 開発速度への影響 | 緩和策 |
|---|---|---|---|
| **MFA 強制** | 高 | 低（初期設定のみ） | セルフサービスパスワードリセット導入 |
| **PIM（JIT アクセス）** | 高 | 中（アクティベーション時間 2〜5 分） | CI/CD パイプライン用サービスプリンシパルは別途設計 |
| **Private Endpoint 強制** | 高 | 高（開発環境での接続複雑性増加） | 開発環境は NSG+サービスエンドポイントで代替可 |
| **Deny ポリシー（Policy Deny）** | 高 | 高（IaC 修正が必要） | Audit から始め、段階的に Deny に移行 |

> セキュリティ施策の導入は段階的に行い、@cloud-governance と連携して Audit → Deny の移行タイミングを調整することを推奨します。
...
セキュリティチームは CAF Plan フェーズの各成果物に以下の形で具体的に貢献します。

### Strategy 成果物 → Plan 成果物への接続

| Strategy 成果物 | セキュリティへの接続 | Plan 成果物 |
|---|---|---|
| **データ分類基準** | 分類ごとのゼロトラスト要件の定義 | セキュリティコントロールマッピング |
| **コンプライアンス要件の優先順位** | 規制対応のセキュリティ実装スコープ | セキュリティ実装ロードマップ（フェーズ別） |
| **リスク受容基準** | 許容できない脅威シナリオの特定 | セキュリティ統制実装計画（Critical 優先） |

### Plan フェーズ主要成果物への貢献

#### 1. クラウド導入計画（Cloud Adoption Plan）

- ゼロトラスト実装のウェーブ別ロードマップ策定（ID → ネットワーク → データの順序）
- Defender for Cloud プランの段階的有効化計画（コスト対効果に基づく優先順位）
- セキュリティゲーティング基準（移行ウェーブごとのセキュリティ承認チェックリスト）

#### 2. スキル準備計画（Skills Readiness Plan）

- Azure セキュリティ専門資格（AZ-500）取得ロードマップの提供
- Sentinel / Defender 運用スキルの内製化計画
- セキュリティレビュアー（Champion）育成プログラムの設計

#### 3. デジタルエステート計画（Digital Estate Plan）

- 既存ワークロードの脅威モデリングと STRIDE 分析による移行リスクスコア付与
- データ分類（機密 / 社外秘 / 公開）の現状マッピングと移行後の保護要件定義
- コンプライアンス対象ワークロードの特定（PCI DSS、GDPR、ISMAP 対象範囲）

#### 4. 組織アライメント（Organizational Alignment）

- セキュリティ RACI（SOC 運用・インシデント対応・脆弱性管理）の定義
- セキュリティチャンピオン制度の設計（各開発チームへの浸透）
- セキュリティレビューゲートの導入計画（DevSecOps パイプラインへの組み込み）
...
@cloud-strategy（Strategy フェーズ）
    ...
    └── セキュリティ ROI 目標 ──→ Defender プランのコスト対効果計画
            │
            ▼
@cloud-security（Plan フェーズ）
    ├── ゼロトラスト実装計画 ──→ @cloud-platform（Landing Zone セキュリティ基盤）
    ├── セキュリティベースライン → @cloud-governance（Azure Policy として実装）
    └── Sentinel 検出ルール ──→ @cloud-operations（監視体制と連携）
```

> @cloud-strategy との連携: Strategy フェーズで定義された「データ分類基準」「コンプライアンス優先順位表」「リスク受容基準」を受領し、Plan フェーズのセキュリティ実装ロードマップの入力として活用します。
